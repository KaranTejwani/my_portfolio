        ...
        <section id="skills" className="relative py-28 md:py-36">
            <div className="section-container relative z-10">
                ...
                <SectionReveal className="mb-12 text-center">
                    ...
                    <p className="text-text-secondary">Technologies and tools I work with daily</p>
                </SectionReveal>
                    ...
                    {skillCategories.map((cat) => (
                        <button
                            ...
                            className={`px-6 py-2 rounded-full text-sm font-medium transition-all duration-300 ${activeCategory === cat.id
                                    ...
                                    : 'border border-border text-text-secondary hover:text-text-primary hover:border-accent/40 bg-bg-surface'
                                }`}
                        >
                            {cat.title}
                        </button>
                ...
                <SectionReveal delay={0.15}>
                    <div className="obsidian-card p-6 md:p-8 mb-12 border hover:border-accent transition-colors max-w-4xl mx-auto">
                        <h3 className="text-xl font-bold text-text-primary font-display mb-8 text-center">{activeSkills.title}</h3>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            {activeSkills.skills.map((skill, index) => (
                                <motion.div
                                    ...
                                    className="flex flex-col items-center gap-4 p-5 rounded-2xl bg-bg-primary border border-border hover:border-accent/40 hover:shadow-glow-cyan transition-all duration-300 group"
                                >
                                    <div className="relative">
                                        <ProgressRing level={skill.level} color={skill.color} />
                                        <div className="absolute inset-0 flex items-center justify-center">
                                            <skill.icon size={22} style={{ color: skill.color }} className="group-hover:scale-110 transition-transform duration-300" />
                                        </div>
                                    </div>
                                    <div className="text-center">
                                        <div className="text-sm font-semibold text-text-primary group-hover:text-accent transition-colors">{skill.name}</div>
                                        <div className="text-xs text-text-muted mt-1">{skill.level}% Proficiency</div>
                                    </div>
                                </motion.div>
                            ))}
                        </div>
